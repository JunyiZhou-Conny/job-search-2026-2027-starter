...
- Prefer the Junyi-supervised Perfect Resume for Polar apply, plus the two-page master as inventory, over creating a completely new resume for every job. Per-role family routing is postponed.
...
- Browser execution has two environments. Cursor cloud Computer Use, and Polar on Junyi's machine. Polar reads `generated/polar/runtime/POLAR_RUNTIME.md`. Polar Local apply starts on authenticated Jobright recommendations. Use **Apply with Autofill**, then Quick Edit / Select All / Generate My Resume / Apply Now. The Jobright extension owns autofill. Do not click Simplify Copilot Autofill on that path. After Autofill, run the fast validation pass on the form DOM, not the extension sidebar: identity (First Name, Last Name, application email), work authorization, eligibility-critical, required-empty-or-error, required legal. Populated routine fields with no error and no known failure class are trusted, not re-read (`knowledge/polar_operator.yaml` `autofill.fast_validation_pass`). Polar may retrieve verification codes from the application Outlook inbox. Do not abandon a recoverable email OTP. User-only remaining: SMS on the Mac when Outlook has no code, hardware 2FA, CAPTCHA after a normal attempt, phone-app push. `/home/polar/PREFERENCES.md` and Polar site notes on jobright.ai are Mac leftovers; a mailbox-cannot-be-read line there is stale. Polar finishes remaining required fields from facts, Submits, confirms on the employer page, then answers Yes / I applied only after confirmation. Sheet lookups are targeted. `READY_*` rows are inventory, not silent FIFO. Pending PREFERENCES candidates keep a `pref_YYYYMMDD_NNN` id until a resolution row is on `main`.
