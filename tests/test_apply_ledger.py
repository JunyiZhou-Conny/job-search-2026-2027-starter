...
class TestApplyLedger(unittest.TestCase):
    ...
    def test_prioritized_harness_identity_and_form_blockers(self):
        ...
        china = dict(MINIMAL_FORM, required_corrections=["sponsorship_answer_no", "citizenship_china"])
        pre = self._pre(form=china)
        self.assertEqual(pre["verdict"], "pass")
        self.assertEqual(pre["required_corrections"], ["sponsorship_answer_no", "citizenship_china"])
