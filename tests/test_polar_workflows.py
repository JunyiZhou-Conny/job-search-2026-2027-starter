...
class TestGeneratedWorkflows(unittest.TestCase):
    ...
    def test_apply_priority_and_simplify_contracts(self):
        ...
        self.assertIn("neither generated nor Perfect Resume / JZ_Resume_2027.pdf can be attached", text)
        ...
        self.assertIn("17. Update the Sheet after every meaningful stage", text)
        self.assertIn("Do not use the browser as the Sheet API.", text)
        self.assertIn("Reach Polar Jobs through the Google connector.", text)
    ...
    def test_phase_three_forbids_autonomous_merge(self):
        ...
        self.assertIn("status: production", text)
        ...
        self.assertIn("Do not flip or rewrite control key github_write_canary.", text)
        self.assertIn("Leave every [Polar Production] Issue open.", text)
        self.assertIn("prefer the newest title date", text)
        self.assertIn("newest learning_reports row by report_date", text)
        self.assertNotIn("latest America/New_York date", text)
        self.assertIn("Do not implement or merge pull/149.", text)
        self.assertIn("ChatGPT review is not a run gate.", text)
        self.assertIn("cannot create a branch or open a PR", text)
    ...
    def test_apply_post_autofill_is_fast_validation_pass(self):
        text = read_workflow("apply-ready-jobs")
        self.assertRegex(text, r"(?m)^workflow_version: 2026-09-16\.apply-runtime-sheet-io\+[0-9a-f]{12}$")
        ...
        self.assertIn("Required future-sponsorship widget: Yes. Required H-1B-named widget: Yes.", text)
        self.assertNotIn("Required future-sponsorship widget: No. Required H-1B-named widget: No.", text)
        ...
        for name in WORKFLOW_RENDERERS:
            self.assertRegex(read_workflow(name), r"(?m)^workflow_version: 2026-09-16\.apply-runtime-sheet-io\+", name)
