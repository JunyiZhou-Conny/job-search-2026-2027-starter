...
class TestPolarRuntime(unittest.TestCase):
    ...
    def test_identity_facts_agree(self):
        ...
        self.assertIn("Historical broad visa-sponsorship mapping: No.", text)
        self.assertIn("Execution: leave_unresolved.", text)
        self.assertIn("Future sponsorship required (standing fact): True", text)
        self.assertIn("Those conflict. Leave a broad sponsorship widget empty", text)
        ...
        self.assertIn("visa_sponsorship: leave unresolved.", text)
