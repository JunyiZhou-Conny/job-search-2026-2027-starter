...
class TestPolarRuntime(unittest.TestCase):
    ...
    def test_identity_facts_agree(self):
        ...
        self.assertNotIn("Future sponsorship required (standing fact): False", text)
        ...
        self.assertIn("Earliest full-time start: 2027-02-16", text)
        self.assertIn("Eligible to begin employment immediately: Yes", text)
        self.assertIn("eligible_to_begin_employment_immediately: Yes.", text)
        self.assertNotIn("conditional_on_start_date", text)
        ...
        self.assertNotIn("visa_sponsorship: No.", text)
    ...
    def test_fast_validation_pass_replaces_full_form_audit(self):
        ...
        self.assertIn("Required currently-authorized with an unknown or non-bool fact (not_yet_authorized_pending_opt_start): leave the field and BLOCK that job only.", text)
        self.assertIn("Required sponsorship-to-begin is No.", text)
        ...
        self.assertIn("sponsorship_no_on_future_sponsorship_widget: wrong value No.", text)
        self.assertNotIn("repeat_key autofill_sponsorship_yes_on_future_sponsorship_widget", text)
        self.assertNotIn("sponsorship_yes_on_future_sponsorship_widget: wrong value Yes.", text)
    ...
    def test_fast_validation_keeps_the_safeties(self):
        ...
        self.assertIn("Required future-sponsorship widget: Yes.", text)
        self.assertNotIn("Required future-sponsorship widget: No.", text)
        self.assertIn("H-1B-named widget: Yes", text)
        self.assertIn("Citizenship country (form and fact): China", text)
    ...
    def test_policy_revision_is_apply_runtime_sheet_io(self):
        import yaml

        operator = yaml.safe_load(
            (ROOT / "knowledge" / "polar_operator.yaml").read_text(encoding="utf-8")
        )
        self.assertEqual(operator["policy_revision"], "2026-09-16.apply-runtime-sheet-io")
        ...
        self.assertEqual(known["sponsorship_no_on_future_sponsorship_widget"]["wrong_value"], "No")
        self.assertEqual(known["sponsorship_no_on_future_sponsorship_widget"]["upstream_status"], "unknown")
        self.assertNotIn("sponsorship_yes_on_future_sponsorship_widget", known)
