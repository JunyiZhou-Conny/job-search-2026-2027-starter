...
class TestLearningAndDiscoverPath(unittest.TestCase):
    ...
    def test_learning_still_sees_outcomes_and_forbids_old_entry(self):
        ...
        self.assertIn("this_workflow=apply-agent-jobs", agent)
        self.assertIn("If resume:", agent)
        self.assertIn("Do not attach to apply-ready-jobs.", agent)
        self.assertIn("Do not compile 100 as the active cap.", agent)
        self.assertIn("not an all-day hourly grind", agent)
        self.assertIn("wake_slots: 08:00, 09:00", agent)
        self.assertIn("intended_cron_et: 0 8,9 * * *", agent)
        self.assertIn("wake_timezone: unknown", agent)
        self.assertIn("owner_daily_shape: unconfirmed", agent)
        self.assertIn("schedule: two_slots_intended_disabled", agent)
        self.assertNotIn("20 * * * *", agent)
        self.assertIn("start_again_after_batch: blocked_until_observe", agent)
        self.assertIn("Never Start", agent)
        self.assertEqual(operator["apply_entry"], "jobright_recommendations")
        self.assertEqual(operator["schedules"]["apply_agent_jobs"]["cron_et"], "")
        self.assertFalse(operator["schedules"]["apply_agent_jobs"]["enabled"])
