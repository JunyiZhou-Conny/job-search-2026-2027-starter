...
| `resumes/` | Inventory in `resumes/base/`. Polar ingest is `resumes/Perfect Resume/perfect_resume.pdf`. Registry in `data/resume_versions.csv` |
...
- Polar prefers a resume already visible on the ATS widget. If Polar must upload a file, use `resumes/Perfect Resume/perfect_resume.pdf`.
- Do not upload `resumes/base/JZ_resume.pdf`. Family one-pagers stay BUILD artifacts. Polar does not pick among them when Copilot cannot attach a resume.
