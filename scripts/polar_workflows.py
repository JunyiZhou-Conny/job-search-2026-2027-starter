...
def render_chatgpt(operator: Dict[str, Any]) -> str:
    ...
    return _lines(
        [
            ...
            f"chatgpt_required: {contract['chatgpt_required']}",
            f"chatgpt_role: {contract['chatgpt_role']}",
            "Do not schedule this Workflow.",
...
def render_cursor(operator: Dict[str, Any]) -> str:
    contract = maintenance_contract()
    dispositions = ", ".join(MAINTENANCE_DISPOSITIONS)
    priorities = ", ".join(PERFORMANCE_PRIORITY)
    return _lines(
        [
            _open_files(),
            _secrets_ban(),
            _lease_block("cursor-production-maintenance", operator),
            "## Maintenance contract",
            "",
            f"chatgpt_required: {contract['chatgpt_required']}",
            f"chatgpt_role: {contract['chatgpt_role']}",
            f"cursor_reads_report_directly: {contract['cursor_reads_report_directly']}",
            f"classify_with: {contract['classify_with']}",
            f"report_title_pattern: {contract['report_title_pattern']}",
            f"recommended_cron_et: {contract['recommended_cron_et']}",
            f"dispositions: {dispositions}",
            f"performance_priority: {priorities}",
            ...
            f"   Disposition is one of: {dispositions}.",
            ...
            f"   Interrogate when: {MAINTENANCE_PLAYBOOK_WHEN['interrogate']}",
            "7. Arena only when at least two credible fixes have meaningful tradeoffs.",
            f"   Arena when: {MAINTENANCE_PLAYBOOK_WHEN['arena']}",
            "8. Architect only when the lesson needs a new durable shape or plane boundary.",
            f"   Architect when: {MAINTENANCE_PLAYBOOK_WHEN['architect']}",
            ...
            "## Performance",
            "",
            f"Rank time sinks in this order: {priorities}.",
