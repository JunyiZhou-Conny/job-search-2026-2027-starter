...
`apply-ready-jobs` is one worker. Before it upserts `PARTIAL`, it queries `run_log` for any `apply-ready-jobs` or `grok-apply-jobs` row that is `PARTIAL` with blank `ended_at`. `start_apply_run_action` returns `NO_WORK` when another apply is live. Polar and Grok share that gate. Do not acquire `polar_browser`. Do not create `grok_browser`.
