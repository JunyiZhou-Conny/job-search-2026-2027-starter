...
`apply-ready-jobs` is one worker. Before it upserts `PARTIAL`, it queries `run_log` for any `apply-ready-jobs` or `grok-apply-jobs` row that is `PARTIAL` with blank `ended_at`. `start_apply_run_action` returns `NO_WORK` when that row is still live (`started_at` younger than `work_claim.ttl_minutes`). A crashed PARTIAL older than that TTL, or with unparseable `started_at`, is `stale_close`: write `ended_at` and `FAILED` via `stale_apply_close_fields`, then continue so abandoned-claim recovery can run. Polar and Grok share that gate. `grok-production-learning-daily` is not an apply and must not call `start_apply_run_action`. Do not acquire `polar_browser`. Do not create `grok_browser`.
