...
| `apply-agent-jobs` | manual, `enabled: false` | Draft two-slot Polar Agent on `/agent` (08:00 and 09:00). Not all-day hourly. Wake timezone unknown. Polar cron is ET. First cap 3. Do not enable while `apply-ready-jobs` is live. Do not press Start. |
